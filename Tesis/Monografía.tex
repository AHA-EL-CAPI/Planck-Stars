...
\begin{equation}
g_{tt} = 1 - \frac{2mr^2}{(r^2 + g^2)^{3/2}} = 0.
\end{equation}
...
¡OJO! Por qué l corresponde a la longitud de planck

!OJOal final de la sección debe ser claro la necesidad de pasar a la sección de la siguiente sección
...
\begin{equation}
g_{tt} \underset{r \to 0}{\sim} 1 - \frac{r^2}{l^2} + \mathcal{O}(r^5)
\end{equation}
y por otro, sus invariantes de curvatura
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.4\textwidth]{fhaywardAnalysis}
	\caption{Posibles casos de agujeros negros para la métrica de Hayward con para un valor fijo del parámetro $l$ y diferentes valores del parámetro $m$.}
	\label{fig: fhayward analysis}
\end{figure}

Al analizar los ceros de $g_{tt}$ revela una masa crítica de $m = $
